...
HeroAI::HeroAI(Hero* h): hero(h) {
	...
	Comands =
	{
		...
		t('i',{make_shared<Shoot>(hero, Point(0, -1), shared_ptr<Shoot::finc_t>(new Shoot::finc_t(&generator)))}),
		t('j',{make_shared<Shoot>(hero, Point(-1, 0), shared_ptr<Shoot::finc_t>(new Shoot::finc_t(&generator)))}),
		t('k',{make_shared<Shoot>(hero, Point(0, 1), shared_ptr<Shoot::finc_t>(new Shoot::finc_t(&generator)))}),
		t('l',{make_shared<Shoot>(hero, Point(1, 0), shared_ptr<Shoot::finc_t>(new Shoot::finc_t(&generator)))})
	};
}
